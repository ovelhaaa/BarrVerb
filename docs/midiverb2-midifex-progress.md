...
- **Resumo técnico (curto):** validado e consolidado o contrato de chamada dos adaptadores DECOMPILED em Web e ESP32 com passagem explícita de `lfo1`/`lfo2` (`uint32`) para as assinaturas C-style dos efeitos (`input`, `out`, `DRAM`, `ptr`, `lfo1`, `lfo2`), sem alterar comportamento de áudio nesta iteração.
